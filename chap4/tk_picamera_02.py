...
def rec_vdo():
    ...
    # 라벨 Movie Saved 생성
    label3 = tk.Label(root, text = 'Movie Saved')
    # 라벨 배치
    label3.pack()
...
with picamera.PiCamera() as camera:
    ...
    # 라벨 rotation 생성
    label2 = tk.Label(root, text = 'rotation')
    # 라벨 배치
    label2.pack()

    for text, mode in rotation:
        # 회전 각도 선택용 라디오 버튼 생성
        rb2 = tk.Radiobutton(root, text = text, variable = rot, value = mode)
        # 라디오 버튼 배치
        rb2.pack(anchor = 'w')
    ...
    # 촬영 시작 버튼 작성
    b = tk.Button(root, text = "Movie", width = 10, command = rec_vdo)
    # 버튼 배치
    b.pack()
